...
'''
Usage: 
    **ALTERED FROM ORIGINAL PURPOSE**
    python3 excelntdonut.py --x86bin file.bin --x64bin file2.bin --obfuscate --sandbox

Options:
    --x86bin 'file.bin'    Takes a precombiled x86 bin /shell code file to parse.
    --x64bin 'file.bin'    Takes a precompiled x64 bin / shell code file to parse.
...
Input File:
    Precompiled shellcode files these can be generated with donut or msfvenom.
...
def main():
    ...
    parser.add_argument("--x64bin", dest="x64bin", help="path to x64 .bin file", required=True)
    parser.add_argument("--x86bin", dest="x86bin", help="path to x86 .bin file", required=True)
    ...
    if not os.path.exists(args.x64bin):
        print(f"[!] Could not open or read files, {args.x64bin}")
        sys.exit()
    ...
    x86Shellcode, x86Size, x86Count = generateShellcode(args.x86bin,'x86')
    x64Shellcode, x64Size, x64Count = generateShellcode(args.x64bin,'x64')
...
def generateShellcode(file, arch):
